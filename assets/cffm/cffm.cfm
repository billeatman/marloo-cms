<cfparam name="page_group" type="string">
<!---
<cfoutput>#application.path.groups##page_Group#</cfoutput>
<cfabort>
--->
<!--- cffmfolders are exception folders --->
<CFIF NOT isDefined("session.auth.groups.#page_Group#") AND NOT isDefined("SESSION.cffmfolders.#page_Group#")>
...
<cffunction name="ListToQuery" access="public" returntype="query">
    ...
    <cfargument name="lstArg" type="string" required="yes" />
   
    <cfset var qTemp = QueryNew(cols) />
    <cfloop list="#lstArg#" index="lstItem">
        <cfset QueryAddRow(qTemp) />
        <cfset QuerySetCell(qTemp,"#Col#",lstItem) />
    </cfloop>
...
<cfinvoke component="#cffm#" method="init">
    ...
    <!--- Use the site root if we are building a path for an exception folder --->
    <cfif isDefined("SESSION.cffmfolders.#page_Group#")>
        <cfinvokeargument name="includeDir" value="#application.path.siteroot##SESSION.cffmfolders[page_Group]#">
        <!--- includeDirWeb = web path to the directory specified above. --->
        <cfinvokeargument name="includeDirWeb" value="#application.web.siteroot##SESSION.cffmfolders[page_Group]#">
	<cfelse>
        <cfinvokeargument name="includeDir" value="#application.path.groups##page_Group#">
        <!--- includeDirWeb = web path to the directory specified above. --->
        <cfinvokeargument name="includeDirWeb" value="#application.web.groups##page_Group#">
	</cfif>
	<!--- disallowedExtensions = file extensions you don't want people to upload --->
	<cfinvokeargument name="disallowedExtensions" value="cfm,cfml,cfc,dbm,dbml,php,php3,php4,php5,asp,aspx,pl,plx,pls,cgi,jsp,pif,scr,vbs,exe,db,js,lck,htm,html">
	<cfinvokeargument name="hiddenExtensions" value="cfm,cfml,cfc,dbm,dbml,php,php3,php4,php5,asp,aspx,pl,plx,pls,cgi,jsp,pif,scr,vbs,exe,db,js,lck">
	<!---
	// allowedExtensions = 
	...
	// images or something.
	--->
	<cfinvokeargument name="allowedExtensions" value="jpg,gif,png,txt,xls,doc,pdf,ppt,zip,mp3">
    ...
    <cfinvokeargument name="editableExtensions" value="txt">
	<!---
	// overwriteDefault = 
...
<cfscript>
	...
	if (not DirectoryExists(cffm.includeDir)) {
		...
		// WE - 08-29-2014
		DirectoryCreate(cffm.includeDir);
	}
	...
	if (variables.action eq "delete") {
		if (variables.deleteFilename contains "/" or variables.deleteFilename contains "\") {
			...
			if (variables.deleteResults.errorCode neq 0) 
			{
				...
				relocate(cffm.cffmFilename & "?subdir=" & urlEncodedFormat(variables.subdir) & "&page_group=" & urlEncodedFormat(page_group));
			}
		}
		variables.action = "list";
		...
		if (variables.editFilename contains "/" or variables.editFilename contains "\") {
			...
			try {
				variables.image = ImageRead(variables.imagePath);
			...
			if (NOT imageLoaded)
			{
				...
				try {
					...
					//	variables.image.resize(variables.resizeWidthValue, variables.resizeHeightValue, yesNoFormat(variables.preserveAspect), yesNoFormat(variables.cropToExact));
						// We must be preserving aspect of width or height are zero
						if (variables.resizeWidthValue eq 0 or variables.resizeHeightValue eq 0){
							variables.preserveAspect = true;
						}
						if (yesNoFormat(variables.preserveAspect) eq 'Yes'){
							if (variables.resizeWidthValue == 0){
								ImageScaleToFit(variables.image, '', variables.resizeHeightValue);
							}else if (variables.resizeHeightValue == 0){ 
								ImageScaleToFit(variables.image, variables.resizeWidthValue, '');
							}else{
								ImageScaleToFit(variables.image, variables.resizeWidthValue, variables.resizeHeightValue);
							}
							// Not working properly
							if (yesNoFormat(variables.cropToExact) eq 'Yes'){
								ImageCrop(variables.image, 0, 0, variables.resizeWidthValue, variables.resizeHeightValue);
							}
						}else{
							ImageResize(variables.image, variables.resizeWidthValue, variables.resizeHeightValue);	
						}
					...
					//	variables.image.crop(variables.cropStartX, variables.cropStartY, variables.cropWidthValue, variables.cropHeightValue);
						ImageCrop(variables.image, variables.cropStartX, variables.cropStartY, variables.cropWidthValue, variables.cropHeightValue);
					}
					DebugOutput("Writing image");
					//variables.image.save(variables.imagePath,95);
					ImageWrite(variables.image, variables.imagePath, .95);
					imageWritten = true;
				} catch(Any e) {
					// cffmdump(e);
				...
				if (not imageWritten)
				{
					...
					relocate(cffm.cffmFilename & "?action=manipulateForm&subdir=" & urlEncodedFormat(variables.subdir) & "&editFilename=" & urlEncodedFormat(variables.editFilename) & "&page_group=" & urlEncodedFormat(page_group));
				}
			}
		}
	}
...
<cfif cffm.templateWrapperAbove neq "">
	...
	<cfsetting enablecfoutputonly="yes">
<cfelse>
	<cfoutput><HTML><HEAD><META HTTP-EQUIV="PRAGMA" CONTENT="NO-CACHE">
<TITLE>CFFM File Manager version <CFOUTPUT>#cffm.version#</CFOUTPUT></TITLE></HEAD><BODY></cfoutput>
...
<cfelseif variables.editorType eq "mce">
	...
	//	srcInput = window.opener.win2.document.getElementById('<cfif variables.EDITOR_RESOURCE_TYPE eq "file">href<cfelseif variables.EDITOR_RESOURCE_TYPE eq "flash">file<cfelse>src</cfif>');		
		id = '<cfif variables.EDITOR_RESOURCE_TYPE eq "file">href<cfelseif variables.EDITOR_RESOURCE_TYPE eq "flash">file<cfelse>src</cfif>'
	//	srcInput.value = fileUrl;

	//	window.close() ;
	//	window.opener.win2.focus() ;
		callbackclose(fileUrl, id);
	}
	</script>
	</cfoutput>
</cfif>
...
<p class="cffm_location">
<!--->	<table>
	<tr>
		<td>
			<img src="#cffm.iconPath#/document_open_64_h.png">
		</td>
		<td> --->
			#cffm.resourceKit.Msg.t1#:  #variables.workingDirectoryWeb#
<!---		</td>
	</tr>
	</table> --->
</p></cfoutput>
...
<cfif variables.action eq "viewSource" or variables.action eq "edit">
	<cfoutput><a href="#cffm.cffmFilename#?subdir=#urlEncodedFormat(variables.subdir)#&page_group=#urlEncodedFormat(page_group)#"><b>#cffm.resourceKit.Msg.t14#</b></A></cfoutput>
	<P>
	<form method="post" action="<cfoutput>#cffm.cffmFilename#</cfoutput>">
	<input type="hidden" name="page_Group" value="<cfoutput>#page_Group#</cfoutput>">
...
<cfelseif action eq "commitChanges">
	...
	<cfoutput>
		...
		<p><a href="#cffm.cffmFilename#?subdir=#urlEncodedFormat(variables.subdir)#&page_group=#urlEncodedFormat(page_group)#"><b>#cffm.ResourceKit.Msg.t14#</b></A></p>
	</cfoutput>
...
<cfelseif action eq "manipulateForm">
	...
	<cfset variables.imageHeight = ImageGetHeight(variables.image)>  
	<cfset variables.imageWidth = ImageGetWidth(variables.image)> 
	<cfoutput>
	...
	<cfelse>
		<p>Changes are being made to a temporary copy of your original image.  
			<a href="#cffm.cffmFilename#?subdir=#urlEncodedFormat(variables.subdir)#&editFilename=#urlEncodedFormat(variables.editFilename)#&action=commitChanges&page_group=#urlEncodedFormat(page_group)#" id="commitChanges"><b>#cffm.ResourceKit.Msg.t64#</b></A>
			<a href="#cffm.cffmFilename#?subdir=#urlEncodedFormat(variables.subdir)#&editFilename=#urlEncodedFormat(variables.editFilename)#&action=undoChanges&page_group=#urlEncodedFormat(page_group)#" id="undoChanges"><b>#cffm.ResourceKit.Msg.t65#</b></A>
		</p>
	</cfif>
	...
	<input type="hidden" name="page_Group" value="#page_Group#">
	<input type="hidden" name="subdir" value="#variables.subdir#">
	<input type="hidden" name="editFilename" value="#variables.editFilename#">
	...
	--->
	<input type="checkbox" name="preserveAspect" checked value="1">
...
<cfelseif action eq "renameForm">
	<cfoutput>
	<a href="#cffm.cffmFilename#?subdir=#urlEncodedFormat(variables.subdir)#&page_group=#urlEncodedFormat(page_group)#"><b>#cffm.ResourceKit.Msg.t14#</b></A><p>
...
<cfelseif action eq "uploadForm">
	<cfoutput>
		<a href="#cffm.cffmFilename#?subdir=#urlEncodedFormat(variables.subdir)#&page_group=#urlEncodedFormat(page_group)#"><b>#cffm.ResourceKit.Msg.t14#</b></A>
		<p>
	</cfoutput>
...
<cfelseif action eq "viewzip">
	<cfoutput><a href="#cffm.cffmFilename#?subdir=#urlEncodedFormat(variables.subdir)#&page_group=#urlEncodedFormat(page_group)#"><b>#cffm.ResourceKit.Msg.t14#</b></A><p></cfoutput>
	<form method="post" action="<cfoutput>#cffm.cffmFilename#</cfoutput>">
	<input type="hidden" name="page_Group" value="<cfoutput>#page_Group#</cfoutput>">
...
<input type="file" name="uploadFile1">&nbsp<input type="submit" <!--->class="button"---> value="<cfoutput>#cffm.resourceKit.buttonText.t5#</cfoutput>"><br>
...
<cfoutput><a href="#cffm.cffmFilename#?subdir=#urlEncodedFormat(subdir)#&action=uploadForm&page_group=#urlEncodedFormat(page_group)#">#cffm.resourceKit.Msg.t25#</a></cfoutput></cfif>
...
<cfoutput>
<a href="#cffm.cffmFilename#?page_group=#urlEncodedFormat(page_group)#"><b>#cffm.resourceKit.Msg.t27#</b></a></li>&nbsp; | &nbsp;
<a href="#cffm.cffmFilename#?subdir=#variables.subdir#&page_group=#urlEncodedFormat(page_group)#"><b>#cffm.resourceKit.Msg.t28#</b></a></li>
</cfoutput>
</div>

<div class="cffm_filediv">
...
<cfset variables.linkToFile = cffm.cffmFilename & "?subdir=" & ListDeleteAt(variables.subdir,listlen(variables.subdir,"/"),"/") & "&page_group=" & urlEncodedFormat(page_group)>
<cfoutput><tr>
<td><a href="<cfoutput>#variables.linkToFile#</cfoutput>" target="_self"><img src="<cfoutput>#cffm.iconPath#/</cfoutput>folder_up.png" border="0"></a></td>
<td width="100%">
...
<!--- could this be slowing stuff down --->
<!--- <cfdump var="#cffm.hiddenExtensions#" expand="no">
<cfdump var="#variables.dirlist#" expand="no"> --->
<cfquery name="sortedDirList" dbtype="query">
	select *, lower(name) as sortname
	from variables.dirlist
	where name not like '__TEMP__%' 
	<cfloop list="#cffm.hiddenExtensions#" index="list">
    and name not like '%#trim(list)#'
    </cfloop>
	order by type, sortname
</cfquery>
<!--- <cfdump var="#sortedDirList#"> --->
<cfoutput query="sortedDirList">
<cfsilent>
<cfset cnt = cnt + 1>
...
if (listLen(name,".") gt 1) {
	...
	if (listFind(cffm.editableExtensions, variables.extension) gt 0 and type eq "file") {
		...
		variables.fileIcon = "documenticon.png";
...
if (type eq "dir") {
	...
	variables.fileIcon = "folder_closed.png";
	variables.linkTarget = "_self";
	variables.linkToFile = cffm.cffmFilename & "?subdir=";
	if (variables.subdir eq "") {
		variables.linkToFile = variables.linkToFile & name & "&page_group=" & urlEncodedFormat(page_group);
	} else {
		variables.linkToFile = variables.linkToFile & variables.subdir & "/" & name & "&page_group=" & urlEncodedFormat(page_group);
	}
} else {
	...
	if (listFind("gif,jpg,png",variables.extension) gt 0)
	{
		...
		variables.fileIcon = "imgicon.png";
		if (variables.extension eq "jpg" or variables.extension eq "png")
		{
			variables.editableImage = 1;
		}
		// Added by William Eatman
	/*		try
			{
			   variables.image = ImageRead(cffm.createServerPath(variables.subdir,name));
			   variables.imageinfo = ImageInfo(image);
			   variables.dimensions = Imageinfo.width & "x" & imageinfo.height;	
			} catch(Any e) {
	...
	if (listFind("pdf",variables.extension) gt 0)
	{
		variables.previewLink = "#variables.workingDirectoryWeb#/#name#"; 
		variables.fileIcon = "pdficon2.png";
		if (variables.extension eq "pdf")
		{
			variables.editableImage = 0;
		}
	}
	...
	{	// editorType is empty when CFFM is being used as a plain ol' file manager.
		...
		if ( variables.EDITOR_RESOURCE_TYPE eq "image" AND listFindNoCase("jpg,gif,png",extension) eq 0 )
		{
			...
			//variables.linkToFile="javascript:top.Ext.MessageBox.alert( "", '#cffm.resourceKit.Msg.t32#.');";				
...
<tr>
<cfif isdefined("variables.linkToFile")>
<td><a href="#variables.linkToFile#" target="#variables.linkTarget#"><img src="#cffm.iconPath#/#variables.fileIcon#" border="0"></a></td>
<td width="100%">
	<a href="#variables.linkToFile#" target="#variables.linkTarget#">#name#</a><cfif type eq "file"><!--- >&nbsp;<a class="previewLink" href="#cffm.cffmFilename#?action=download&subdir=#urlEncodedFormat(subdir)#&downloadFilename=#urlEncodedFormat(name)#&page_group=#urlEncodedFormat(page_group)#">[#cffm.resourceKit.Msg.t47#]</a> ---></cfif><cfif variables.editorType neq "" AND variables.previewLink neq "">&nbsp;<a class="previewLink" target=_blank href="#variables.previewLink#">[#cffm.resourceKit.Msg.t48#]</a></cfif>
</td>
<cfelse>
<td><a><img src="#cffm.iconPath#/#variables.fileIcon#" border="0"></a></td>
<td width="100%">
	<a>#name#</a><cfif type eq "file"><!--- >&nbsp;<a class="previewLink" href="#cffm.cffmFilename#?action=download&subdir=#urlEncodedFormat(subdir)#&downloadFilename=#urlEncodedFormat(name)#&page_group=#urlEncodedFormat(page_group)#">[#cffm.resourceKit.Msg.t47#]</a> ---></cfif><cfif variables.editorType neq "" AND variables.previewLink neq "">&nbsp;<a class="previewLink" target=_blank href="#variables.previewLink#">[#cffm.resourceKit.Msg.t48#]</a></cfif>
</td>
</cfif>
<!--- <td><a><img src="#cffm.iconPath#/#variables.fileIcon#" border="0"></a></td>
<td width="100%">
...
<td class="actionLinks">
	<cfif not cffm.readOnly><a href="javascript:if(confirm('Delete #cffm.getPathType(cffm.createServerPath(variables.subdir,name))# \'#replace(name,"'","\'","ALL")#\'?')){window.location.href='#cffm.cffmFilename#?action=delete&page_group=#urlEncodedFormat(page_group)#&subdir=' + escape('#replace(variables.subdir,"'","\'","ALL")#') +'&deleteFilename=' + escape('#replace(name,"'","\'","ALL")#');}">#cffm.resourceKit.Msg.t50#</a>&nbsp;<a href="#cffm.cffmFilename#?action=renameForm&page_group=#urlEncodedFormat(page_group)#&subdir=#urlEncodedFormat(variables.subdir)#&editFilename=#urlEncodedFormat(name)#">#cffm.resourceKit.Msg.t51#</a>&nbsp;<a href="#cffm.cffmFilename#?action=copymoveForm&page_group=#urlEncodedFormat(page_group)#&subdir=#UrlEncodedFormat(variables.subdir)#&editFilename=#urlEncodedFormat(name)#">#cffm.resourceKit.Msg.t52#</a>&nbsp;<cfif variables.editable eq 1><a href="#cffm.cffmFilename#?action=edit&page_group=#urlEncodedFormat(page_group)#&subdir=#urlEncodedFormat(variables.subdir)#&editFilename=#urlEncodedFormat(name)#">#cffm.resourceKit.Msg.t53#</a>&nbsp;</cfif></cfif><cfif variables.editable eq 1><a href="#cffm.cffmFilename#?action=viewSource&page_group=#urlEncodedFormat(page_group)#&subdir=#urlEncodedFormat(variables.subdir)#&editFilename=#urlEncodedFormat(name)#">#cffm.resourceKit.Msg.t54#</a>&nbsp;</cfif><cfif not cffm.readOnly and cffm.allowUnzip and variables.zipfile eq 1><a href="#cffm.cffmFilename#?action=viewzip&page_group=#urlEncodedFormat(page_group)#&subdir=#urlEncodedFormat(variables.subdir)#&editFilename=#urlEncodedFormat(name)#">#cffm.resourceKit.Msg.t55#</a>&nbsp;</cfif><cfif not cffm.readOnly and variables.editableImage><a href="#cffm.cffmFilename#?action=manipulateForm&page_group=#urlEncodedFormat(page_group)#&subdir=#urlEncodedFormat(variables.subdir)#&editFilename=#urlEncodedFormat(name)#">#cffm.resourceKit.Msg.t56#</a>&nbsp;</cfif>
</td>
</tr>
</cfoutput>
</table>
...
#variables.totalFiles# <cfif variables.totalFiles eq 1>#cffm.resourceKit.Msg.t37#<cfelse>#cffm.resourceKit.Msg.t38#</cfif>
#cffm.resourceKit.Msg.t36# #variables.totalDirectories# <cfif variables.totalDirectories IS 1>#cffm.resourceKit.Msg.t34#<cfelse>#cffm.resourceKit.Msg.t35#</cfif>.
</p>
</cfoutput>
<!--- >#cffm.resourceKit.Msg.t39# #NumberFormat(variables.totalSize,"_,___")# bytes.
</cfoutput>
...
<cfelse>
	<cfoutput><a href="#cffm.cffmFilename#?subdir=#variables.subdir#&showTotalUsage=1&page_group=#urlEncodedFormat(page_group)#">#cffm.resourceKit.Msg.t45#</a></cfoutput>
</cfif> --->
...
<cfoutput>
<p>
#cffm.resourceKit.Msg.t46# <a href="http://www.webworksllc.com/cffm">CFFM v.#cffm.version#</A>
</p>
</cfoutput>
